...
/*
<!--
<div id="broken-webbooker-activity" data-bind="with: WebBooker.ActivityView.activity, visible: WebBooker.ActivityView.show() && WebBooker.ActivityView.invalidLanguage()" style="display:none">
	<div class="error">
...
<div id="webbooker-activity" data-bind="if: WebBooker.ActivityView.activity(), visible: WebBooker.ActivityView.show()" style="display:none">
	<div class="header">
		<h3 data-bind="html: WebBooker.ActivityView.activity().title"><?php echo $wb['activity']['title'];?></h3>
	</div>
	<div class="cb"></div>
	<div class="ribbonFold"></div>
	<div id="webbooker-activity-actions" class="clearfix">
		<div class="return">
			<a title="<?php _e('Return to Search Results','arez'); ?>" href="<?php echo (isset($wb['wb_url'])) ? $wb['wb_url'] : ''; ?>/#/Search" data-bind="attr: { href: WebBooker.searchUrl }, click: WebBooker.ActivityView.analyticsContinueShopping" class="buttonGray">
	...
	<div id="webbooker-activity-media">
		<div id="slideshow-noimages" data-bind="visible: !WebBooker.ActivityView.slideshow().length">
			<div class="inner">
				<?php _e('No slideshow images to display.','arez'); ?>
			</div>
		</div>
		
		<div id="activityCarousel" class="carousel slide carousel-fit" data-bind="visible: WebBooker.ActivityView.slideshow().length">
		  <ol class="carousel-indicators" data-bind="foreach: WebBooker.ActivityView.slideshow">
		  ...
		  <!-- Carousel items -->
		  <div class="carousel-inner"  data-bind="foreach: WebBooker.ActivityView.slideshow">
			  	<!-- ko if: $index() == 0 -->
		  ...
		  <!-- Carousel nav -->
			<a id="activityFullScreen" class="btn btn-primary" data-toggle="modal" data-target="#imageModal" data-local="#activityCarouselFullScreen" data-bind="click: WebBooker.ActivityView.viewFullSize">
	...
	<div id="webbooker-activity-summary" class="gradient-blue clearfix">
		<div class="more-info">
			<div class="right">
				<!-- ko if:WebBooker.ActivityView.activity().duration && WebBooker.ActivityView.activity().duration != '0' -->
				<strong><?php _e('Duration','arez'); ?>: </strong><span data-bind="text: WebBooker.ActivityView.activity().duration === 'false' ? '' : WebBooker.ActivityView.activity().duration"><?php echo $wb['activity']['duration']; ?></span><br />
				<!-- /ko -->
				<strong><?php _e('Days','arez'); ?>: </strong><span data-bind="text: WebBooker.ActivityView.days"></span><br />
				<span data-bind="visible: WebBooker.Agent.user_id() > 0"><strong><?php _e('Root Activity ID','arez'); ?>: </strong>#<span data-bind="text: WebBooker.ActivityView.activity().activityID"><?php echo $wb['activity']['activityID'];?></span></span>
			</div>

			<div class="middle">
				<strong><?php _e('Location','arez'); ?>: </strong><span id="activity-location" data-bind="text: __(WebBooker.ActivityView.activity().destination)()"><?php echo $wb['activity']['destination'];?></span>
				<!-- ko if:WebBooker.ActivityView.activity().address_lat -->
	...
	<div class="row-fluid">
		<div id="webbooker-activity-little">
			<div class="content">
				<span data-bind="html: WebBooker.ActivityView.activity().shortDesc"><?php echo $wb['shortDesc'];?></span>
				<?php
		...
		<div id="webbooker-activity-main">
			<div class="content">
				<div class="description" data-bind="html: WebBooker.ActivityView.activity().description"><?php echo $wb['activity']['description'];?></div>
				<!-- ko if: WebBooker.ActivityView.activity().instructions -->
					<p><strong><?php _e('Special Instructions','arez'); ?>:</strong></p>
					<div class="specIns" data-bind="html: WebBooker.ActivityView.activity().instructions"><?php echo $wb['activity']['instructions'];?></div>
				<!-- /ko -->
	...
	<div id="webbooker-activity-map" class="row-fluid" data-bind="visible: WebBooker.ActivityView.activity().address_lat">
		<p><strong><?php _e('Address','arez'); ?>:</strong> <span id="activity-map-addr" data-bind="html: WebBooker.ActivityView.activity().address"><?php echo $wb['activity']['address'];?></span></p>
